...
from cellworld_game import Agent, Point, Model, AgentState, CoordinateConverter, Mouse, Robot, CellWorldLoader, Polygon, Visibility
import enum


class PeekingBotEvadePreyData:
    def __init__(self):
        self.puffed = False
...
class PeekingBotEvade(Model):
    ...
    def __init__(self,
                 ...
                 peeking_occlusion_ratio: float = 1.25,
                 peeking_time_out: float = 0.5
                 ):
        ...
        self.peeking_occlusion_ratio = peeking_occlusion_ratio
        self.peeking_time_out = peeking_time_out
        ...
        self.generate_peeking_visibility()
        ...
        self.prey_data = PeekingBotEvadePreyData()

    def generate_peeking_visibility(self):
        self.peeking_visibility_color = (255, 255, 128)
        arena_transformation: cw.Transformation = self.loader.world.implementation.space.transformation
        peeking_cell_transformation: cw.Transformation = self.loader.world.implementation.cell_transformation
        peeking_cell_transformation.size = peeking_cell_transformation.size * self.peeking_occlusion_ratio
        self.peeking_occlusions = [Polygon.regular(center=cell.location.get_values(),
                                                   diameter=peeking_cell_transformation.size,
                                                   angle=arena_transformation.rotation + peeking_cell_transformation.rotation,
                                                   sides=self.loader.world.configuration.cell_shape.sides)
                                   for cell
                                   in self.loader.world.cells.occluded_cells()]
        self.peeking_visibility = Visibility(arena=self.loader.arena, 
                                             occlusions=self.peeking_occlusions)

        self.peeking_visibility_polygon: Polygon = None
        self.peeking_timer: cw.Timer = None

        def render_peeking_visibility(surface, coordinate_converter):
            if self.render_agent_visibility == "":
                return
            if self.peeking_visibility_polygon is None:
                return 
            self.peeking_visibility_polygon.render(surface=surface,
                                                   coordinate_converter=coordinate_converter,
                                                   color=self.peeking_visibility_color)
        if self.render:
            self.view.add_render_step(render_peeking_visibility, z_index=25)

    def __update_state__(self,
                         delta_t: float = 0):

        self.peeking_visibility_polygon = self.peeking_visibility.get_visibility_polygon(src=self.predator.state.location,
                                                                                         direction=self.predator.state.direction,
                                                                                         view_field=self.predator.view_field)
        ...
        if self.use_predator and self.puff_cool_down <= 0:
            ...
            predator_might_see_prey = self.line_of_sight["predator"]["prey"]

            if predator_might_see_prey:
                if self.peeking_timer is None:
                    #print("IT CAN'T SEE YOU YET")
                    self.peeking_timer = cw.Timer(self.peeking_time_out)
            else:
                self.peeking_timer = None

            predator_sees_prey = self.peeking_visibility_polygon.intersects(self.prey.body_polygon)
            
            if predator_sees_prey or (self.peeking_timer and self.peeking_timer.time_out()):
                #print("NOW IT CAN SEE YOU")
